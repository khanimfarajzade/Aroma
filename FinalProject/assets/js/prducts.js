fetch("http://localhost:3000/products")
  ...
  .then((data) => {
    data.forEach((x, y) => {
      const cardTrendContainer = document.querySelector(".cardTrendContainer");
      ...
      //add css
      cardTrendContainer.classList.add("cardTrendContainer");
      ...
      h4.innerHTML = `<a href="./product.html" target="_blank">${data[y].name}</a>`;
      span.innerHTML = `$${data[y].price}`;

      //append
      cardTrendContainer.appendChild(cardTrend);
